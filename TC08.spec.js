...
test('handle js simple alert with playwright',async({page})=>{
    ...
    page.on('dialog',async simpleAltert=>{
        await expect(simpleAltert.message()).toContain('I am a JS Alert')
...
test("Handle propmt alert with playwright",async({page})=>{
    await page.goto('https://the-internet.herokuapp.com/javascript_alerts')
    page.on('dialog', async propmtAlert=>{
        await expect(propmtAlert.message()).toContain('I am a JS prompt')
        await expect(propmtAlert.type()).toContain('prompt')
        await propmtAlert.accept('Rohan')
    })
    await page.locator('button[onclick="jsPrompt()"]').click()
    await expect(page.locator("#result")).toHaveText('You entered: Rohan')
    await page.waitForTimeout(5000)
})
